...
class Config:
    ...
    # Risk settings
    rr_ratio:   float = 1.5   # Lowered — hits TP faster

    # Order Block settings
    ob_lookback:       int   = 80    # Wide lookback = always finds a valid OB
    ob_min_body_ratio: float = 0.3   # Accept weaker OB candles
    ob_proximity_pct:  float = 1.0   # Wide entry zone inside OB

    # Signal filters
    swing_lookback: int   = 3        # Smaller = more swings detected
    min_fvg_pct:    float = 0.01     # Smaller FVG minimum = more FVGs
    rsi_period:     int   = 7
    rsi_overbought: float = 75.0     # Higher = entries in stronger momentum
    rsi_oversold:   float = 25.0     # Lower = same reason
    body_ratio_min: float = 0.35     # Smaller body candles accepted
    cooldown_bars:  int   = 2        # Signals can fire closer together
    session_filter: bool  = False  # False = scan 24/7
    require_mss:    bool  = True     # MSS confirmation back on for quality
